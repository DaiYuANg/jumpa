...
func registerRBACEndpoints(api *httpx.Group, roleSvc application.RoleService, groupSvc application.PermissionGroupService, permSvc application.PermissionService) {
	httpx.MustGroupGet(api, "/roles", func(ctx context.Context, input *ListResourceInput) (*dynamicOutput, error) {
		...
		for _, it := range list { if input.Q == "" || containsFold(it.Name, input.Q) { filtered = append(filtered, toRoleDTO(it)) } }
	...
	httpx.MustGroupGet(api, "/permission-groups", func(ctx context.Context, input *ListResourceInput) (*dynamicOutput, error) { list, err := groupSvc.ListPermissionGroups(ctx); if err != nil { return nil, err }; filtered := make([]permissionGroupDTO, 0, len(list)); if ids := parseIDsCSV(input.ID); len(ids) > 0 { idSet := collectionset.NewSet(ids...); filtered = toPermissionGroupDTOs(lo.Filter(list, func(it iamdomain.PermissionGroup, _ int) bool { return idSet.Contains(it.ID) })); return &dynamicOutput{Body: ok(filtered)}, nil }; for _, it := range list { if input.Q == "" || containsFold(it.Name, input.Q) { filtered = append(filtered, toPermissionGroupDTO(it)) } }; p := paginate(filtered, input.Page, input.PageSize); return &dynamicOutput{Body: okPage(p.Items, p.Total, p.Page, p.PageSize)}, nil }, huma.OperationTags("permission-groups"))
	...
	httpx.MustGroupGet(api, "/permissions", func(ctx context.Context, input *ListResourceInput) (*dynamicOutput, error) { list, err := permSvc.ListPermissions(ctx); if err != nil { return nil, err }; filtered := make([]permissionDTO, 0, len(list)); if ids := parseIDsCSV(input.ID); len(ids) > 0 { idSet := collectionset.NewSet(ids...); filtered = toPermissionDTOs(lo.Filter(list, func(it iamdomain.Permission, _ int) bool { return idSet.Contains(it.ID) })); return &dynamicOutput{Body: ok(filtered)}, nil }; for _, it := range list { if input.Q == "" || containsFold(it.Name, input.Q) || containsFold(it.Code, input.Q) { filtered = append(filtered, toPermissionDTO(it)) } }; p := paginate(filtered, input.Page, input.PageSize); return &dynamicOutput{Body: okPage(p.Items, p.Total, p.Page, p.PageSize)}, nil }, huma.OperationTags("permissions"))
