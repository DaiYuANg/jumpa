...
func registerUserEndpoints(api *httpx.Group, userSvc application.UserService, userRoleSvc application.UserRoleService, principalSvc application.AuthPrincipalService) {
	httpx.MustGroupGet(api, "/users", func(ctx context.Context, input *ListResourceInput) (*dynamicOutput, error) { // truncated behavior parity
		if validIDs := parseInt64IDsCSV(input.ID); len(validIDs) > 0 {
			res := collectionx.NewListWithCapacity[userDTO](len(validIDs))
	...
	httpx.MustGroupPatch(api, "/users/{id}", func(ctx context.Context, input *PatchUserInput) (*dynamicOutput, error) {
		...
		u, err := userSvc.Update(ctx, id, iamdomain.UpdateUserInput{Name: input.Body.Name, Email: input.Body.Email, Age: input.Body.Age})
		if err != nil {
			return nil, err
		}
		if u.IsAbsent() {
		...
		user := u.MustGet()
		if err := principalSvc.UpsertAuthPrincipal(ctx, user.ID, user.Email); err != nil {
			return nil, err
		}
...
func registerRBACEndpoints(api *httpx.Group, roleSvc application.RoleService, groupSvc application.PermissionGroupService, permSvc application.PermissionService) {
	...
	httpx.MustGroupGet(api, "/roles/{id}", func(ctx context.Context, input *ByIDInput) (*dynamicOutput, error) {
		it, err := roleSvc.GetRole(ctx, input.ID)
		if err != nil {
			return nil, err
		}
		if it.IsAbsent() {
		...
		return &dynamicOutput{Body: ok(toRoleDTO(it.MustGet()))}, nil
	}, huma.OperationTags("roles"))
	...
	httpx.MustGroupPatch(api, "/roles/{id}", func(ctx context.Context, input *PatchRoleInput) (*dynamicOutput, error) {
		it, err := roleSvc.UpdateRole(ctx, input.ID, input.Body.Name, input.Body.Description, input.Body.PermissionGroupIDs)
		if err != nil {
			return nil, err
		}
		if it.IsAbsent() {
	...
	httpx.MustGroupGet(api, "/permission-groups/{id}", func(ctx context.Context, input *ByIDInput) (*dynamicOutput, error) {
		it, err := groupSvc.GetPermissionGroup(ctx, input.ID)
		if err != nil {
			return nil, err
		}
		if it.IsAbsent() {
		...
		return &dynamicOutput{Body: ok(toPermissionGroupDTO(it.MustGet()))}, nil
	}, huma.OperationTags("permission-groups"))
	...
	httpx.MustGroupPatch(api, "/permission-groups/{id}", func(ctx context.Context, input *PatchPermissionGroupInput) (*dynamicOutput, error) {
		it, err := groupSvc.UpdatePermissionGroup(ctx, input.ID, input.Body.Name, input.Body.Description)
		if err != nil {
			return nil, err
		}
		if it.IsAbsent() {
	...
	httpx.MustGroupGet(api, "/permissions/{id}", func(ctx context.Context, input *ByIDInput) (*dynamicOutput, error) {
		it, err := permSvc.GetPermission(ctx, input.ID)
		if err != nil {
			return nil, err
		}
		if it.IsAbsent() {
		...
		return &dynamicOutput{Body: ok(toPermissionDTO(it.MustGet()))}, nil
	}, huma.OperationTags("permissions"))
	...
	httpx.MustGroupPatch(api, "/permissions/{id}", func(ctx context.Context, input *PatchPermissionInput) (*dynamicOutput, error) {
		it, err := permSvc.UpdatePermission(ctx, input.ID, input.Body.Name, input.Body.Code, input.Body.GroupID)
		if err != nil {
			return nil, err
		}
		if it.IsAbsent() {
